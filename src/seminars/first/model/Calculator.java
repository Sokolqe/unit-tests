...
public class Calculator {
    ...
    public static double calculatingDiscount(double purchaseAmount, int discountAmount) {
        if (discountAmount >= 0 && discountAmount < 100 && purchaseAmount > 0) {
            ...
            throw new ArithmeticException("Скидка больше допустимой.");
        } else if (discountAmount < 0) {
            throw new ArithmeticException("Скидка не может быть меньше 0");
        } else if (purchaseAmount <= 0) {
            throw new ArithmeticException("Цена не может быть меньше или равна нулю");
        } else {
            throw new IllegalStateException("Неверный тип данных");
        }
    }
}